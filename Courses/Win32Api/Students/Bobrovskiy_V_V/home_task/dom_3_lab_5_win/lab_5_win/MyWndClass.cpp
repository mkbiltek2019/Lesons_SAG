...
int MyWndClass::x=0;
int MyWndClass::y=0;
MOUSEHOOKSTRUCT* MyWndClass::mstruct;
//----------------------
void MyWndClass::random_window_pos(const HWND hWnd, const LPARAM lParam){
		int x=(LONG)HIWORD(lParam);
		char buf[10]; itoa(x,buf,10);
		int y=(LONG)LOWORD(lParam);
		char buf1[5]; itoa(y,buf1,10);
		strcat(buf,","); strcat(buf,buf1);
		SetWindowText(hWnd, buf); 

		HDC hDCScreen = GetDC(NULL);//get desctop context
	    int Hor = GetDeviceCaps(hDCScreen,  HORZRES);
		 int Vert = GetDeviceCaps(hDCScreen,  VERTRES);
	    ReleaseDC(NULL, hDCScreen);

		srand(time(0));
		if(x<Hor-100&&y<Vert-100) ::MoveWindow(hWnd,x+rand()%80+20,y+rand()%80,100,100,true);		
		else{ x=rand()%y; y=rand()%x; ::MoveWindow(hWnd,x-rand()%80,y-rand()%80,100,100,true); }
};
//----------------------
LRESULT CALLBACK MyWndClass::WndProc(const HWND hWnd,const UINT uMsg,const WPARAM wParam,const LPARAM lParam){
	switch(uMsg){
	case WM_NCHITTEST:{
		random_window_pos(hWnd,lParam);
			}break;
	case WM_CREATE:	{	
...
int MyWndClass::CreateWnd(){
	...
	hMainWnd=CreateWindow(this->szClassName.c_str(),"Hallo man ",WS_VISIBLE,
		CW_USEDEFAULT,0,100,100,(HWND)NULL,
