...
namespace DialogManger.ImageManger
{   
    public class ColorDialogManager
    {
    ...
    public class ImageManager
    {
            ...
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        loadedImageCopy = 
                            ExtractBitmapFromOpenedFile(Image.FromFile(openFileDialog.FileName));
        ...
        private void SaveBitmapByExtension(string fullFileName, Image curentImage, ImageFormat imageFormat)
        {
            ...
            {
                Bitmap imageToSave = (Bitmap)ExtractBitmapFromOpenedFile(curentImage);

                EncoderParameters encoderParameters = new EncoderParameters(1);
                encoderParameters.Param[0] =
                    new EncoderParameter(System.Drawing.Imaging.Encoder.Quality, imageQuality);

                imageToSave.MakeTransparent(Color.White);//white color will be transparent

                imageToSave.MakeTransparent(Color.White);

                imageToSave.Save(fullFileName, GetEncoder(imageFormat), encoderParameters);

                imageToSave.Dispose();
            }
        }

        private Image ExtractBitmapFromOpenedFile(Image curentImage)
        {
            Bitmap bitmapToSave = new Bitmap(curentImage.Width, curentImage.Height, PixelFormat.Format32bppArgb);
