    ...
    public class HttpDownloadClient
    {
        ...
        public HttpDownloadClientStatus Status
        {
            ...
            private set
            {
                ...
                {
                    status = value;
                    this.OnStatusChanged(new StatusChangedArgs(Index));
                }
            }
        }
        ...
        /// </summary>
        public HttpDownloadClient(int index, string url, string downloadPath)
            : this(index,url, downloadPath, 0)
        {
            this.index = index;
        }
        ...
        public HttpDownloadClient(int index,string url, string downloadPath,
           int startPoint)
            : this(index,url, downloadPath, startPoint, int.MaxValue)
        {
        }
        ...
        public HttpDownloadClient(int index,string url, string downloadPath,
          int startPoint, int endPoint)
            : this( index,url, downloadPath, startPoint, endPoint, 1024, 1048576, 64)
        {
        }

        public HttpDownloadClient(int index,string url, string downloadPath, int startPoint,
        ...
        void Download()
        {
            ...
            try
            {
                ...
                else if (this.Status == HttpDownloadClientStatus.Canceling)
                {
                    ...
                    this.OnDownloadCompleted(new HttpDownloadCompletedEventArgs(
                                   this.DownloadedSize, this.TotalSize, this.TotalUsedTime, ex,Index));
                }
                else
                {
                    this.Status = HttpDownloadClientStatus.Completed;
                    this.OnDownloadCompleted(new HttpDownloadCompletedEventArgs(
                                this.DownloadedSize, this.TotalSize, this.TotalUsedTime, null,Index));
                    return;
                }
        ...
        private void InternalDownloadProgressChanged(int cachedSize)
        {
            ...
            this.OnDownloadProgressChanged(new HttpDownloadProgressChangedEventArgs
                           (this.DownloadedSize + cachedSize, this.TotalSize, speed,Index));
